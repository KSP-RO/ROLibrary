...
namespace ROLib
{
    ...
    /// </summary>
    public class ROLAnimationModule
    {
        public delegate ROLAnimationModule SymmetryModule(PartModule module);
        ...
        public Action<ROLAnimState> onAnimStateChangeCallback;
        ...
        /// </summary>
        private ROLAnimState animationState = ROLAnimState.STOPPED_START;
        ...
        private void loadAnimationState(string persistence)
        {
            ...
            {
                animationState = (ROLAnimState)Enum.Parse(typeof(ROLAnimState), persistence);
            }
            if (animationState == ROLAnimState.PLAYING_BACKWARD)
            {
                animationState = ROLAnimState.STOPPED_START;
                animationPosition = 0f;
            }
            else if (animationState == ROLAnimState.PLAYING_FORWARD)
            {
                animationState = ROLAnimState.STOPPED_END;
                animationPosition = deployLimit;
            }
            else if (animationState == ROLAnimState.STOPPED_END)
            {
                animationPosition = deployLimit;
            }
            else if (animationState == ROLAnimState.STOPPED_START)
            {
                animationPosition = 0f;
            }
        }
        ...
        private void onDeployLimitUpdated(BaseField a, object b)
        {
            this.actionWithSymmetry(m =>
            {
                ...
                {
                    m.stopAnimation();
                    m.setROLAnimState(ROLAnimState.STOPPED_END);
                }
            });
        }
        ...
        {
            if (animationState == ROLAnimState.STOPPED_START || animationState == ROLAnimState.PLAYING_BACKWARD)
            {
                this.actionWithSymmetry(m =>
                {
                    m.setROLAnimState(ROLAnimState.PLAYING_FORWARD);
                    m.updateUIState();
                });
            }
        }
        ...
        {
            if (animationState == ROLAnimState.STOPPED_END || animationState == ROLAnimState.PLAYING_FORWARD)
            {
                this.actionWithSymmetry(m =>
                {
                    m.setROLAnimState(ROLAnimState.PLAYING_BACKWARD);
                    m.updateUIState();
                });
            }
        }
        ...
        public void onToggleAction(KSPActionParam param)
        {
            if (animationState == ROLAnimState.STOPPED_START || animationState == ROLAnimState.PLAYING_BACKWARD)
            {
                setROLAnimState(ROLAnimState.PLAYING_FORWARD);
                updateUIState();
            }
            else
            {
                setROLAnimState(ROLAnimState.PLAYING_BACKWARD);
                updateUIState();
            }
        }
        ...
        public void setupAnimations(AnimationData anims, Transform root, int startLayer)
        {
            ...
            setROLAnimState(animationData.Count <= 0 ? ROLAnimState.STOPPED_END : animationState);
        ...
        public void setROLAnimState(ROLAnimState newState, bool updateCallback = false)
        {
            switch (newState)
            {
                case ROLAnimState.PLAYING_BACKWARD:
                    {
                        setAnimSpeed(-1f);
                        if (animationState == ROLAnimState.STOPPED_END)//enforce play backwards from end
                        {
                ...
                case ROLAnimState.PLAYING_FORWARD:
                    {
                        setAnimSpeed(1f);
                        if (animationState == ROLAnimState.STOPPED_START)//enforce play forwards from beginning
                        {
                ...
                case ROLAnimState.STOPPED_END:
                    {
                        setAnimTime(1, true);
                        stopAnimation();
                        break;
                    }
                case ROLAnimState.STOPPED_START:
                    {
                        setAnimTime(0, true);
                        stopAnimation();
                        break;
        ...
        private void updateUIState()
        {
            ...
            bool deployEnabled = moduleEnabled && (animationState == ROLAnimState.STOPPED_START || animationState == ROLAnimState.PLAYING_BACKWARD);
            bool retractEnabled = moduleEnabled && (animationState == ROLAnimState.STOPPED_END || animationState == ROLAnimState.PLAYING_FORWARD);
        ...
        private void actionWithSymmetry(Action<ROLAnimationModule> action)
        {
            action(this);
    ...
    public class AnimationData
    {
        ...
        {
            deployLabel = node.ROLGetStringValue("deployLabel", "Deploy");
            retractLabel = node.ROLGetStringValue("retractLabel", "Retract");
            toggleLabel = node.ROLGetStringValue("toggleLabel", "Toggle");
            deployLimitActive = node.ROLGetBoolValue("deployLimitActive", false);
            activeEditor = node.ROLGetBoolValue("activeEditor", true);
            activeFlight = node.ROLGetBoolValue("activeFlight", true);
            activeUnfocused = node.ROLGetBoolValue("activeUnfocused", false);
            activeUncommanded = node.ROLGetBoolValue("activeUncommanded", false);
            activeEVAOnly = node.ROLGetBoolValue("activeEVAOnly", false);
            unfocusedRange = node.ROLGetFloatValue("unfocusedRange", 4f);
            oneShot = node.ROLGetBoolValue("oneShot", false);
            looping = node.ROLGetBoolValue("looping", false);
    ...
    public class ModelAnimationData
    {
        ...
        {
            animationName = node.ROLGetStringValue("name");
            speed = node.ROLGetFloatValue("speed", 1f);
            isLoop = node.ROLGetBoolValue("loop", false);
        }
    ...
    public class ModelAnimationDataControl
    {
        ...
        {
            animationName = node.ROLGetStringValue("name");
            animationSpeed = node.ROLGetFloatValue("speed", animationSpeed);
            animationLayer = node.ROLGetIntValue("layer", animationLayer);
            maxDeployTime = node.ROLGetFloatValue("max", maxDeployTime);
        ...
        public bool setMaxTime(float max, ROLAnimState state)
        {
            ...
            for (int i = 0; i < len; i++)
            {
                if (state == ROLAnimState.STOPPED_END)
                {
                    sample(animations[i], animations[i][animationName], maxDeployTime);
                    //no change in state
                }
                else if (state == ROLAnimState.PLAYING_FORWARD)
                {
                ...
                else if (state == ROLAnimState.PLAYING_BACKWARD)
                {
                    float nt = animations[i][animationName].normalizedTime;
                    if (nt > maxDeployTime)
                    {
                ...
                else if (state == ROLAnimState.STOPPED_START)
                {
                    //NOOP
                }
            }

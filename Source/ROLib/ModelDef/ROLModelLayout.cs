...
namespace ROLib
{
    ...
    public static class ROLModelLayout
    {
    ...
    public struct ModelPositionData
    {
        ...
        {
            localPosition = node.ROLGetVector3("position", Vector3.zero);
            localScale = node.ROLGetVector3("scale", Vector3.one);
            localRotation = node.ROLGetVector3("rotation", Vector3.zero);
        }
    ...
    public class ModelDefinitionLayoutOptions
    {
        ...
        public readonly ROLModelDefinition definition;
        public readonly ModelLayoutData[] layouts;

        public ModelDefinitionLayoutOptions(ROLModelDefinition def)
        {
            definition = def;
            if (definition == null) { ROLLog.error("Model definition was null when creating model layout options!"); }
        ...
        {
            return layouts.ROLExists(m => m.name == name);
        }

        public string[] getLayoutNames()
        {
            return ROLUtils.getNames(layouts, m => m.name);
        }
